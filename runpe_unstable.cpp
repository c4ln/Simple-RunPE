...
int main()
{
    ...
    std::cout << "loading file into mem" << std::endl;
    HANDLE hdle = MapFileToMemory("test_prog.exe");
    if(hdle != 0)
    {
        if(RunPortableExecutable(hdle) == 0)
        {
            std::cout << "finished loading into mem and started process" << std::endl;
            return 0;
        }
        else
        {
            std::cout << "error running executable from memory" << std::endl;
        }
    }
    else
    {
        std::cout << "mapfile error" << std::endl;
    }
